...
function startStatusPolling(interval) {
    async function checkStatus() {
        try {
            const response = await fetch('/assets/php_file/checkNewRequest.php');
            const data = await response.json();

            if (data.success && data.data) {
    const currentID = Number(data.data.requestID);  // ensure numeric
    const fullName = data.data.fullName;
    const documentType = data.data.documentType;               
    const previousID = Number(localStorage.getItem("PreviousRequestID")) || 0;

    if (currentID !== previousID) {
        localStorage.setItem("PreviousRequestID", currentID);  // overwrite directly
        showNotification(`${fullName.toUpperCase()} REQUESTED: ${documentType} CERTIFICATE`, "info");
    }
} else {
    console.error("Error fetching status:", data.message || "Invalid response");
}
        } catch (err) {
